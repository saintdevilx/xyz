	...
	void multiply(const vector<ullong> &v, vector<ullong> & output) {
		...
		for(int row = 0; row < size; ++row) 
			for(int col = 0; col < size; ++col) 
				output[row] =(output[row] + (M[row][col] * v[col])%MOD_NUM)%MOD_NUM;
	}
...
private:
	...
	void multiplyWithItself() {
		vector<vector<ullong> > temp = vector< vector<ullong> > (size, vector<ullong>(size, 0));		
		for(int row = 0; row < size; ++row)
			for(int col = 0; col < size; ++col) {
				for(int k = 0; k < size; ++k)
				    temp[row][col] = 
						(temp[row][col] + (M[row][k] * M[k][col] ) %MOD_NUM ) % MOD_NUM;
			}
		...
		for(int row = 0; row < size; ++row)
			for(int col = 0; col < size; ++col) {
				for(int row2 = 0; row2 < size; ++row2)
				    temp[row][col] = 
						(temp[row][col] + (M[row][row2] * orig[row2][col] ) %MOD_NUM ) % MOD_NUM;
			}
		for(int row = 0; row < size; ++row)
			for(int col = 0; col < size; ++col)
				M[row][col] = temp[row][col];
...
void print(vector<ullong> &vec, int n) {
	...
	for(auto it = vec.begin() + vec.size() - n;it != vec.end(); ++it, ++index) {
		printf("%d", *it);
		if(index < vec.size()-1)
			printf(" ");
	}
	for(auto it = vec.begin();it != vec.begin() + vec.size() - n; ++it, ++index) {
		printf("%d", *it);
		if(index < vec.size()-1)
			printf(" ");
	}
